...
int main()
{
    while (1)
    {
        ...
        /*EH for wsa*/if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0)//进行相应的socket库绑定,MAKEWORD(2,2)表示使用WINSOCK2版本
        {
            cerr << "\nFailed to initialize the WinSock2 DLL\n" << "error code: " << WSAGetLastError() << endl;//Cerr通常用于输出错误信息与其他不属于正常逻辑的输出内容
            return -1;
        }

        u_long ulDestIP = inet_addr(ipString);/*将一个点分十进制的IP转换成一个长整数型数（u_long类型）*//*cout << "ulDestIP = " << ulDestIP <<  << endl; */


        /*EH for ulDestIP*/if (ulDestIP == INADDR_NONE)//INADDR_NONE 是个宏定义，代表IpAddress 无效的IP地址。
        {
            //域名解析
        ...
        */
        ...
        /*EH for sockRaw*/if (sockRaw == INVALID_SOCKET)//无效套接字
        {
        ...
        //设置端口属性
        int iTimeout = DEF_ICMP_TIMEOUT;/*3000ms*//*cout<<"测试————"<<"TIMEOUT:"<<iTimeout<<endl;*/
        /*EH for setsockopt*/if (setsockopt(sockRaw, SOL_SOCKET, SO_RCVTIMEO, (char*)&iTimeout, sizeof(iTimeout)) == SOCKET_ERROR)
        ...
        while (!bReachDestHost && iMaxHop--)
        {
            ...
            /*EH for sendto*/if (sendto(sockRaw, IcmpSendBuf, sizeof(IcmpSendBuf), 0, (sockaddr*)&destSockAddr, sizeof(destSockAddr)) == SOCKET_ERROR)
            {
                //如果目的主机不可达则直接退出
            ...
            while (1)
            {
                ...
                /*EH for recvfrom : correct*/if (iReadDataLen != SOCKET_ERROR) //有数据包到达
                {
                ...
                /*EH for recvfrom : WSAETIMEDOUT*/else if (WSAGetLastError() == WSAETIMEDOUT) //接收超时，打印星号
                {
                    cout << setw(9) << '*' << '\t' << "Request timed out." << endl;
                    break;
                }
                /*EH for recvfrom : others*/else
                {
